...
- `credit_wallet_on_verification()` — **new**: on aggregator verification, credits the
  citizen's `wallets.balance` and `eco_points`, writes a `wallet_transactions` row and a
  notification
...
The sign-in screen (`/sign-in`) now presents an explicit **role-portal selector** — pick your
...
### ✅ Built and verified working (live-tested end-to-end today, not just code review)
...
- Live Realtime updates throughout — confirmed today with zero manual refreshes anywhere in
  the six-role chain
...
- **Google Maps address picker** — citizen "Add a new address" now has a real interactive
  map: click/drag a pin or use "Use my location", captures precise `lat`/`lng` on the
  address row, with Places autocomplete search. Collector's "Navigate" button uses those
  coordinates to open real Google Maps turn-by-turn directions instead of a dead link.
  Verified live end-to-end, including on the deployed Vercel domain.
...
2. **Supabase Auth redirect URLs.** Add `https://ecotrace-hazel.vercel.app` (and any custom
   ...
   dashboard-only config, not something scriptable from here. Without it, `/forgot-password`
   → `/reset-password` email links will bounce back to `localhost`.
